...
| **FISK-2** | Assembly (po `SourceType`) + atomarna numeracija (`invoices`/`invoice_items`/`invoice_counters`). **🟡 U TOKU (2026-06-14):** ✅ 2a poslovni identitet prodavca PIB/PDV/IBAN (`aedaf46`), ✅ 2b `vat_rate_key` na 4 tabele artikala (`c412fb0`). ⬜ 2c: invoices šema + atomarna numeracija + assembly (PG funkcija). | `InvoiceDraft` · pgTAP (RLS izolacija + bez rupa u nizu, paralelne narudžbe) |
