...
| **FISK-2** | Assembly (po `SourceType`) + atomarna numeracija (`invoices`/`invoice_items`/`invoice_counters`). **🟡 U TOKU (2026-06-14):** ✅ 2a PIB/PDV/IBAN (`aedaf46`), ✅ 2b `vat_rate_key` na 4 tabele (`c412fb0`), ✅ 2c JEZGRO (`17d3545`): tabele + RLS + `create_invoice_from_items` (PDV grupe po stopi + atomarni redni broj u istoj transakciji + idempotencija) + pgTAP 041 (10 asertacija). ⬜ 2c-readeri: po-izvor funkcije (order_items/folio/spa → items) koje zovu `create_invoice_from_items`; okidač na zatvaranje računa. | `InvoiceDraft` · pgTAP (RLS izolacija + bez rupa u nizu, paralelne narudžbe) ✓ |
