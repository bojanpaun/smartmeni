...
**Ostaje (ODGOĐENO na zahtjev korisnika 2026-07-07):** **Owner/agencijski portal (RENT-2)** — DB temelj postoji
(`rental_owners.user_id`→auth, `owner_id`/`commission_pct` na assets, sve nekorišteno u UI), treba: owners CRUD +
dodjela na asset + obračun provizije (RPC/RLS owner-scoped) + zaseban portal `/:slug/owner` (šablon = StaffPortal:
red-kao-identitet + self-healing user_id/email link) + ns `ownerportal` lazy. **Provizija — odluka nije donesena**
(osnovica base vs total; ko dobija čišćenje; % = agencija zadržava ili vlasnik dobija). **RENT-FLEET vozila**
(`asset_kind='vehicle'` + `/rentacar`) i **booking/iCal integracija (RENT-1)** takođe odgođeni.
Ostalo opciono: fiskal najma (regulatorno). Detalji: [[project-rental-vertical-wip]].
...
- **MAPA/ADRESA/PINOVI (Feature B, DEPLOYOVANO 2026-07-06):** izabran **Leaflet + OpenStreetMap** (bez API
  ključa/naplate). Per-asset `address`/`latitude`/`longitude` (migr. `20260706200000`, RPC-ovi
  `get_available_rental_assets`/`list_rental_assets` dopunjeni geo kolonama). **Admin** (`AssetsPage`): unos adrese
  + interaktivni pin-drop (klik/prevlačenje) u formi smještaja. **/rent:** „Lista | Mapa" toggle — mapa sa
  markerima svih dostupnih smještaja, klik na pin → izbor smještaja (`fitBounds` na sve pinove). Dijeljena
  `components/shared/RentalMap.jsx` (lazy, van init bundle-a). Demo smještaji dobili koordinate (Budva/Kotor).
  i18n: `rentbooking.viewList/viewMap`, `admin.raAddress/raPin/raPinHint` ×7. **Bundle:** javni ns-ovi
  `rentbooking`/`renthub`/`hotellanding` prebačeni na LAZY (page-specific) da init ostane < 155 kB (154.6 kB).
- **POLITIKA PLAĆANJA (RENT-PAY, DEPLOYOVANO 2026-07-06, migr. `20260706210000`):** admin bira **online plaćanje
  (depozit)** ili **plaćanje po dolasku** — ogledalo hotelskog `rate_plans.payment_type`, ali na TENANT nivou
  (`rental_settings.payment_type` `'online'|'on_arrival'`, default **`on_arrival`**) + konfigurabilan
  `rental_settings.deposit_pct` (default 30). `create_rental_booking_public` čita politiku (uklonjen hardkodovan 30%):
  `on_arrival`→depozit 0, `online`→`total*pct%`; vraća `payment_type`. Admin `SettingsPage` sekcija „Plaćanje"
  (izbor + depozit % kad je online + hint). Javni `/rent` „done" korak grana: `on_arrival`→poruka „plaćanje po dolasku"
  (bez dugmeta), `online`+provajder→depozit dugme (dinamičan %), `online` bez provajdera→fallback. Admin `BookingsPage`
  dobio `payment_status` badge. pgTAP 076 (on_arrival→0, online 50%→pct) + 075 ažuriran (eksplicitno online). i18n ×7
  (admin `rsPay*`/`rbPay_*`, rentbooking `payOnArrivalDone`/`noOnlinePayment`/`doneSubArrival`); audit `rental.settings_update`.
- **POLISH BATCH ZAVRŠEN (DEPLOYOVANO 2026-07-07, `3ceb1e3`+`c15d00c`):** (1) **AI prevod landing blokova** na
  `/rentals` hub-u — dodat `rental` u `LANDING_FIELDS` (editor sad stvarno prevodi; ranije no-op) + hub čita `trL`;
  (2) **restoran `/home` cross-link** „Iznajmi smještaj" → `/rentals` (ako tenant ima rental vertikalu); (3) **SEO**
  `useSeo` hook (document.title + meta description) na `/rent` i `/rentals`; (4) **i18n rental-emaila** — `send-rental-email`
  prima `lang` (gost bira jezik; admin šalje tenant default), STRINGS ×7; (5) **rental ModuleHelp** — `content.rental`
  u modulehelp ns ×7 (6 sekcija + 5 savjeta) + ruta `/admin/rental/help`.
