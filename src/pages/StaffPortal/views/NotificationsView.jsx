...
export default function NotificationsView({ restaurantId }) {
  ...
  if (loading) return <div style={{ padding: 24, textAlign: 'center', color: 'var(--c-text-muted)' }}>Učitavanje…</div>

  if (items.length === 0) {
    return <div style={{ padding: 40, textAlign: 'center', color: 'var(--c-text-muted)' }}>Trenutno nema obavještenja.</div>
  }
      ...
      {items.map(a => {
        ...
        return (
          <div key={a.id} className={s.announcementCard} style={{ opacity: expired ? 0.6 : 1 }}>
            <div className={s.announcementTitle}>📢 {a.title}{expired && <span style={{ fontSize: 11, color: 'var(--c-text-muted)', fontWeight: 400 }}> (isteklo)</span>}</div>
