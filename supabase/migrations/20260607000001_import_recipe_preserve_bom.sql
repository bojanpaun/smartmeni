-- Re-import NE resetuje recept postojeće stavke
--
-- ZAŠTO: dosadašnji RPC je za postojeću stavku bezuslovno radio BOM upsert
-- (ON CONFLICT DO UPDATE SET quantity) → vraćao količine na default biblioteke i
-- ponovo dodavao sastojke koje je admin obrisao. To gazi admin izmjene recepta,
-- nekonzistentno s "backfill, ne gazi" logikom za ostala polja.
--
-- POPRAVKA: BOM se popunjava SAMO ako stavka još nema recept (prazan BOM) —
-- novi import ili stavka uvezena prije inventory_pro. Ako stavka već ima
-- sastojke, recept se NE dira. ON CONFLICT → DO NOTHING (defanzivno).
...
BEGIN
  ...
  IF v_existing.id IS NOT NULL THEN
    ...
    -- Backfill samo praznih polja; admin vrijednosti se ne gaze.
  ...
  -- BOM samo uz inventory_pro I samo ako stavka JOŠ NEMA recept (ne gazi izmjene).
  IF v_has_inv THEN
    SELECT EXISTS (
      SELECT 1 FROM public.menu_item_ingredients WHERE menu_item_id = v_menu_item_id
    ) INTO v_has_recipe;
  ...
  RETURN jsonb_build_object(
    ...
    'recipe_skipped',  (v_has_inv AND v_has_recipe),
