-- ================================================================
-- Fix: get_room_packages — ispravan status check za specificne sobe
--
-- Problem s prethodnim fixom: NOT IN ('maintenance','blocked') je
-- dopuštao 'occupied' sobe, a online rezervacije ne postavljaju
-- room_id pa NOT EXISTS uvijek prolazi → plan se prikazivao i kad
-- su sve sobe zauzete.
--
-- Ispravna logika:
--   'available'  → slobodna, bookable ✓
--   'cleaning'   → upravo oslobođena, bookable za buduće datume ✓
--   'occupied'   → gost trenutno tamo → NIJE bookable ✗
--   'maintenance'→ u kvaru → NIJE bookable ✗
--   'blocked'    → blokirana → NIJE bookable ✗
-- ================================================================
...
BEGIN
  ...
  WHERE rp.room_type_id  = p_room_type_id
    ...
    AND (
      ...
      OR
      -- Plan sa specificnim sobama → barem jedna soba je slobodna:
      --   status IN ('available','cleaning') I nema potvrđene rezervacije
      --   koja pokriva traženi period (za admin-dodijeljene rezervacije)
      EXISTS (
        SELECT 1
