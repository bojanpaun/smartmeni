-- ============================================================================
-- RENT: precizna adresa + koordinate po smještaju (mapa/pin). Feature B.
-- rental_assets += address/latitude/longitude; javni RPC-ovi vraćaju lat/lng
-- (za /rent map filter). Demo smještaji dobiju koordinate (Budva/Kotor).
-- ============================================================================
...
COMMENT ON COLUMN public.rental_assets.latitude IS 'Geo pin smještaja (Leaflet/OSM). Postavlja se u admin formi.';

-- get_available_rental_assets → +latitude/longitude (DROP+CREATE, promjena RETURNS TABLE).
...
CREATE FUNCTION public.get_available_rental_assets(
  p_restaurant_id uuid, p_start date, p_end date, p_guests int DEFAULT 1
) RETURNS TABLE (
  asset_id uuid, name text, description text, location_name text, city text,
  base_price numeric, cleaning_fee numeric, min_duration int,
  max_guests int, bedrooms int, beds int, bathrooms int,
  amenities text[], photos text[], latitude numeric, longitude numeric, address text,
  nights int, base_total numeric, tourist_tax numeric, total_amount numeric
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT
    a.id, a.name, d.description, loc.name, loc.city,
    a.base_price, COALESCE(a.cleaning_fee, 0), COALESCE(a.min_duration, 1),
    d.max_guests, d.bedrooms, d.beds, d.bathrooms, COALESCE(d.amenities, '{}'),
    ARRAY(SELECT btrim(u) FROM unnest(string_to_array(COALESCE(d.photo_urls,''), E'\n')) WITH ORDINALITY AS x(u,ord)
          WHERE btrim(u) ~ '^https?://' ORDER BY ord),
    a.latitude, a.longitude, a.address,
    (p_end - p_start), (q.q->>'base_total')::numeric, (q.q->>'tourist_tax')::numeric, (q.q->>'total_amount')::numeric
  ...
  WHERE a.restaurant_id = p_restaurant_id AND a.status = 'active' AND a.asset_kind = 'accommodation'
    AND p_end > p_start
    AND (d.max_guests IS NULL OR d.max_guests >= p_guests)
    AND (p_end - p_start) >= COALESCE(a.min_duration, 1)
    AND NOT EXISTS (SELECT 1 FROM rental_bookings b WHERE b.asset_id = a.id AND b.status <> 'cancelled'
...
-- list_rental_assets → +latitude/longitude/address (DROP+CREATE).
DROP FUNCTION IF EXISTS public.list_rental_assets(uuid);
CREATE FUNCTION public.list_rental_assets(p_restaurant_id uuid)
RETURNS TABLE (
  asset_id uuid, name text, description text, location_name text, city text,
  base_price numeric, pricing_unit text, cleaning_fee numeric, min_duration int,
  max_guests int, bedrooms int, beds int, bathrooms int, amenities text[], photos text[],
  latitude numeric, longitude numeric, address text
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT a.id, a.name, d.description, loc.name, loc.city,
...
-- Demo smještaji dobiju adresu + koordinate (u seed_demo_rental → zadržava se pri noćnom resetu).
...
BEGIN
  ...
  INSERT INTO public.rental_assets (id, restaurant_id, location_id, name, base_price, cleaning_fee, min_duration, address, latitude, longitude) VALUES
    (a1, r, loc,  'Apartman Galeb', 55.00, 20.00, 2, 'Njegoševa 4, Budva', 42.278900, 18.838600),
    (a2, r, loc2, 'Vila Maslina',  180.00, 50.00, 3, 'Dobrota bb, Kotor',  42.438100, 18.741100)
  ON CONFLICT (id) DO NOTHING;
