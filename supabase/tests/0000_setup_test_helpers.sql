-- ============================================================================
-- Setup hook — VENDOROVANI test helperi (basejump supabase_test_helpers v0.0.6)
-- Izvor: https://github.com/usebasejump/supabase-test-helpers
-- ----------------------------------------------------------------------------
-- Ugrađeno direktno (bez dbdev / CREATE EXTENSION) → nula vanjskih zavisnosti,
-- deterministički i lokalno i u CI-ju. Ime '0000_' ga svrstava prvi po abecedi,
-- pa se 'tests' schema i helperi kreiraju prije svih ostalih testova.
-- (Originalni '\echo ... \quit' čuvar je uklonjen jer se ne učitava preko
--  CREATE EXTENSION nego direktno.)
--
-- Daje: tests.create_supabase_user / authenticate_as / get_supabase_uid /
--       clear_authentication / rls_enabled / freeze_time, ...
--
-- Ažuriranje verzije: zamijeni sadržaj ispod novim izdanjem helpera (skini
-- prve 2 linije \echo/\quit), ostatak fajla ostaje isti.
-- ============================================================================
...
END
$$ LANGUAGE plpgsql;
-- ── no-op test (pgTAP traži bar jedan test po fajlu) ────────────────────────
begin;
select plan(1);
select ok(true, 'Setup: test helperi učitani');
select * from finish();
rollback;
